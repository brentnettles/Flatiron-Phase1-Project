fetch('http://localhost:3000/menu')
  ...
  .then(menuData => {
    ...
    renderCategory(menuData["Appetizers"], '.Appetizers');
    renderCategory(menuData["Mains"], '.Mains');
    renderCategory(menuData["Desserts"], '.Desserts');
  })
  .catch(error => {
    console.error('Error fetching or rendering menu:', error);
  });

function renderCategory(categoryItems, categoryClass) {
  const categorySection = document.querySelector(categoryClass);
  if (!categorySection) {
    throw new Error(`Category section not found: ${categoryClass}`);
  }
